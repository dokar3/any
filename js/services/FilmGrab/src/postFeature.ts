...
export function fetchFreshList(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const thumb = getMediumSizeThumb(thumbElement);

    let media: Post.Media[] = null;
    if (thumb.url) {
      media = [Post.Media.photo({ url: thumb.url, aspectRatio: thumb.ratio })];
    }
...
export function search(params: SearchPostsParams): PagedResult<Post[]> {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const thumbElement = item.select(".entry-thumb .entry-thumb-content > img");
    const thumb = getMediumSizeThumb(thumbElement);
...
function getMediumSizeThumb(coverElement: DomElement) {
