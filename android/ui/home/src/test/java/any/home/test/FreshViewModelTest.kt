...
class FreshViewModelTest {
    ...
    private fun uiState(): FreshUiState {
    ...
    fun loadInitialPosts() = runTest {
        ...
        advanceUntilIdle()
        assertEquals(remotePosts, uiState().posts.map { it.raw })
        ...
        advanceUntilIdle()
        assertEquals(localPosts, uiState().posts.map { it.raw })
    ...
    fun initialPostsCaching() = runTest {
        ...
        assertEquals(initialPosts.markInFresh(), uiState().posts.map { it.raw })
    }
    ...
    fun loadMorePosts() = runTest {
        ...
        assertTrue(uiState().isLoadingMorePosts)

        advanceUntilIdle()

        assertEquals(
            page1Posts.markInFresh() + page2Posts,
            uiState().posts.map { it.raw },
        )
    ...
    fun remotePostFieldsUpdating() = runTest {
        ...
        assertEquals(expectedPosts, uiState().posts.map { it.raw })
    }
}